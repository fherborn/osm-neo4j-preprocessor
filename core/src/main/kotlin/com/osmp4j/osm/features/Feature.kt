...
class Value<T>(val name: String)
class Attribute<T>(key: String) : Feature<T>(key)

open class Feature<T>(val key: String) {
    ...
    protected fun value(name: String) = Value<T>(name)
    protected fun attr(name: String) = Attribute<T>(name)
}